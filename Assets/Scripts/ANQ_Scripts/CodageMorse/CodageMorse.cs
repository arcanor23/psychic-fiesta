...
public class CodageMorse : MonoBehaviour
{
    ...
    public void Verification()      // check if the right morse translation was given for each word
    {
        ...
        if (word == 0)
        {
            if (textMorse.text == CodageData.animalMorse) // if equal, it will put the text in green, indicate it has changed, indicate the new word, indicate to display
            {
                textMorse.text = "";
                word = 1;
                displayResult = true;
        ...
        if (word == 1)
        {
            if (textMorse.text ==  CodageData.numberMorse)
            {
                textMorse.text = "";
                word = 2;
                displayResult = true;
        ...
        if (word == 2)
        {
            if (textMorse.text == CodageData.directionMorse)
            {
                textMorse.text = "";
                word = 3;
                displayResult = true;
    ...
    public void TooLong()       // check if the input morse word is too long compared to right morse word to find
    {
        ...
        {
            if (textMorse.text.Length >= CodageData.animalMorse.Length)      // the input will be stopped and the text will be red colored
            {
        ...
        {
            if (textMorse.text.Length >= CodageData.numberMorse.Length)
            {
                textMorse.color = Color.red;
                isBlocked = true;
        ...
        {
            if (textMorse.text.Length >= CodageData.directionMorse.Length)
            {
                textMorse.color = Color.red;
                isBlocked = true;
            }
